function [Notes,endtime] = midiInfo_polyphony(midi,outputFormat,tracklist,verbose)
%% Tato funkce vychazi z pozmenene funkce "midiInfo" navrzene panem 
% Ken Schutte. Typ nastroje pro polyfonii je nacten primo z MIDI souboru.
% Informace o typu nastroje je ulozena v devatem sloupci promenne NOTES a
% odpovida cislu nastroje podle standardu MIDI (viz. tabulka 
% MIDI_instrument_table.pdf).
%
...
instrument_type=[]; % instrument init value
...
for i=1:length(tracklist)
  ...
  for msgNum=1:length(midi.track(tracknum).messages)
    ...
    if type==192
           if isempty(instrument_type)
               instrument_type=[chan, data];
           else
               fv=find(instrument_type(:,1)==chan);
               if isempty(fv)
                   instrument_type(end+1,:)=[chan, data];
               else
                   instrument_type(fv,2)=data;
               end
           end
%            instrument_type=data;
%            disp(['Channel ' num2str(chan+1) ': instrument no. ' num2str(data+1)]); % korekce +1 kvuli pocitani od nuly
    end
    ...
    if (midimeta==1 && type==144 && data(2)>0)
      % note on:
      if any(instrument_type(:,1)==chan)     
          Notes(end+1,:) = [tracknum chan data(1) data(2) seconds 0 msgNum -1 instrument_type(instrument_type(:,1)==chan,2)];
      else
          error('Prekladany midi soubor je chybny. Obsahuje nedefinovany nastroj !!! ');
      end
	...
	Notes(ind,6) = seconds;
	Notes(ind,8) = msgNum;
	Notes(ind,9) = instrument_type(instrument_type(:,1)==chan,2);
      end
